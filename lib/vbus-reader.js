...
// How long to wait for the initial TCP+handshake before giving up
 ...
 *   'connect'     – TCP connection established (including after auto-reconnect)
 *   'unavailable' – connection lost; NetLiveTransceiver is retrying in the background
 *   'disconnect'  – explicitly disconnected via disconnect()
 */
class VbusReader extends EventEmitter {
  ...
  /**
   * Start reading. Returns a Promise that resolves once the initial TCP
   * handshake completes, or rejects after CONNECT_TIMEOUT_MS. After the
   * initial connection succeeds, NetLiveTransceiver handles auto-reconnect.
   */
  connect() {
    ...
    this._transceiver = new NetLiveTransceiver({
      ...
      onConnectionStateChange: (state) => {
        if (state === "connected") {
          ...
          // Only on the first drop, not on subsequent 'reconnecting' transitions
    ...
    const tx = this._transceiver;
    const connectPromise = tx.connect();
    ...
    return Promise.race([connectPromise, timeoutPromise]).then(
      ...
      async (err) => {
        ...
        if (this._transceiver === tx) this._transceiver = null;
        throw err;
      },
    );
  }
